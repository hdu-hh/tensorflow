...
// TestSessionConfig exercises SessionOptions using [pbs.MustMarshal].
func TestSessionConfig(t *testing.T) {
	graph := NewGraph()
	const want int32 = 14
	c := _Const(graph, "Const", want)
	opts := SessionOptions{Config: pbs.MustMarshal(&pbs.ConfigProto{IntraOpParallelismThreads: 1})}
	...
	if got := output[0].Value().(int32); got != want {
		t.Fatalf("Got %v, want %v", got, want)
	}
}
